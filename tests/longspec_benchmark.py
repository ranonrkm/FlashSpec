...
parser.add_argument('--temperature', type=float, default=0.1, help='Target sample temperature.')
...
parser.add_argument('--draft_tp', action='store_true', help='Whether to compile the model.')
...
if args.draft_tp or not use_tp:
    draft = LMBackend_Draft(dtype=DTYPE, device=DEVICE)
    draft.load_model(draft_checkpoint_path, use_tp=args.draft_tp, rank_group=args.rank_group, group=global_group)
    if args.compile:
        draft.compile()
    draft.setup_caches(max_batch_size=BATCH_SIZE, max_seq_length=MAX_LEN)
...
for step, batch in tqdm(enumerate(dataloader), total=num_eval_steps):
    ...
    while terminal == False:
        ...
        if args.draft_tp or not use_tp:
            for i in range(args.gamma+1):
                if i == args.gamma:
                    draft.inference(tokens_buffer[:, -1].view(-1,1))
                    break
                tokens_buffer[:,i+1:i+2] = draft_sample(draft.inference(tokens_buffer[:, i].view(-1,1)))
        else:
            if rank == 0:
                for i in range(args.gamma+1):
                    if i == args.gamma:
                        ...
                        break
                    tokens_buffer[:,i+1:i+2] = draft_sample(draft.inference(tokens_buffer[:, i].view(-1,1)))
            dist.broadcast(tokens_buffer, src=0, group=global_group)
    ...
    # Verify loop
        ...
        if args.draft_tp or not use_tp:
            draft.cachelens = draft.cachelens - args.gamma - 1
            draft.cachelens += accept_nums.flatten()
        else:
            if rank == 0:
                draft.cachelens = draft.cachelens - args.gamma - 1
                draft.cachelens += accept_nums.flatten()
        ...
        if num_nodes.max() + 1 >= args.M:
            terminal = True
        # Put Bonus tokens to the tokens buffer
        if not terminal:
...
# for step, batch in tqdm(enumerate(dataloader), total=num_eval_steps):
#     input_ids = batch[0][:,:1500].to(DEVICE)
#     terminal = False
#     tokens_buffer= torch.zeros((BATCH_SIZE, args.gamma+1), device=DEVICE).long()
#     output = torch.zeros(BATCH_SIZE, MAX_LEN, device=DEVICE).long()
#     output[:, :input_ids.shape[1]] = input_ids
...
#     print("total time :{:.5f}s, time per iter :{:.5f}s, decoding step: {}, large model step: {}".format(total_time, total_time / target_steps, num_gen_tokens, target_steps))
#     if benchmark:
#         print("target time :{:.5f}s, draft time :{:.5f}s, verify loop : {}, avg generate len per sentence: {}".format(draft_time/target_steps, target_time / target_steps, verify_loop/target_steps, num_gen_tokens/target_steps/BATCH_SIZE))
#     if step == 0:
#         total_time = 0.0
#         num_gen_tokens = 0
#         target_steps = 0
